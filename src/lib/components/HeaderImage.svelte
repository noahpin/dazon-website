...
<div class="header-image-wrap" style={heightOverride ? `height: ${heightOverride};` : ""}>
    ...
    <div class="bar">
        ...
        <GbStatus status="ic" customStatus={"Group Buy May 31st"}></GbStatus>
    </div>
</div>
...
<style>
    ...
    .blur-image {
        ...
        transform: translate(calc(var(--blursize) * -1), calc(var(--blursize) * -1));
        mask-image: linear-gradient(to bottom, transparent calc(100% - 220px), black);
    }
    .bar {
        margin: 20px 30px;
        width: auto; left: 0px; right: 0px; max-width: 100%;
        padding-bottom: 10px;
        position: absolute;
        bottom: 0;
        left: 0;
